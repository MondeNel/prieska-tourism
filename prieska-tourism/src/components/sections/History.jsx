...
const History = () => {
  return (
    <section id="history" className="py-16 px-4 max-w-7xl mx-auto bg-white">
      <SectionTitle 
      ...
      <div className="relative">
        <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-prieska-sand hidden md:block"></div>
        ...
        {historyTimeline.map((item, index) => (
          ...
          >
            <div className="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-prieska-terracotta rounded-full border-4 border-white shadow-md z-10 hidden md:block"></div>
            
            <div className="md:w-1/2 p-4 md:p-6">
              <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border-t-4 border-prieska-terracotta group">
                <div className="flex items-center gap-2 mb-3">
                  <span className="bg-prieska-terracotta text-white text-xs font-bold px-3 py-1 rounded-full shadow-sm">
                    {item.year}
                  </span>
                  <span className="text-sm font-semibold text-prieska-river uppercase tracking-wide">
                    {item.era}
                  </span>
                </div>
                <h3 className="text-xl font-serif font-bold mb-3 group-hover:text-prieska-terracotta transition">{item.title}</h3>
