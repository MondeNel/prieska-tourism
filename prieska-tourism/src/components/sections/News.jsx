...
const News = () => {
  ...
  return (
    <section id="news" className="py-16 px-4 max-w-7xl mx-auto bg-prieska-sand/30 dark:bg-gray-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-md hover:shadow-lg transition">
          ...
          <div className="p-6">
            <span className="text-prieska-terracotta dark:text-prieska-terracotta text-sm font-semibold uppercase">
              {featureNews.category}
            </span>
            <h3 className="text-2xl font-serif font-bold mt-2 mb-3 text-gray-800 dark:text-white">{featureNews.title}</h3>
            <p className="text-gray-600 dark:text-gray-300 mb-4">{featureNews.excerpt}</p>
            <div className="flex items-center justify-between">
              <span className="text-gray-500 dark:text-gray-400 text-sm">{featureNews.date}</span>
              <button className="text-prieska-river dark:text-prieska-river font-medium hover:underline">
                Read More →
              </button>
            </div>
          </div>
        ...
        <div className="space-y-6">
          {sideNews.map((item) => (
            <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-md hover:shadow-lg transition flex flex-col sm:flex-row lg:flex-col">
              ...
              <div className="p-4">
                <span className="text-prieska-terracotta dark:text-prieska-terracotta text-xs font-semibold uppercase">
                  {item.category}
                </span>
                <h4 className="font-serif font-bold text-lg mt-1 mb-2 text-gray-800 dark:text-white">{item.title}</h4>
