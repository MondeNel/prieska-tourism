...
const Navbar = ({ switchFeed, activeFeed, openModal }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 dark:bg-gray-900/90 backdrop-blur-md shadow-lg py-1' : 'bg-white dark:bg-gray-900 py-1.5'}`}>
      ...
      {isOpen && <div className="fixed inset-0 top-[49px] z-40 bg-black/50" onClick={() => setIsOpen(false)} />}
      <div className={`fixed top-[49px] right-0 bottom-0 w-72 max-w-[85vw] bg-white dark:bg-gray-900 shadow-2xl z-50 transform transition-transform duration-300 overflow-y-auto ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
