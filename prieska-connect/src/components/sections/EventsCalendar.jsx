    ...
    <div className="space-y-3">
        ...
        {!showComposer ? (
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-gray-100 dark:bg-gray-700 rounded-lg px-3 sm:px-4 py-2 sm:py-2.5 text-[11px] sm:text-sm text-gray-700 dark:text-gray-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-prieska-terracotta resize-none"
              rows={3}
            />
            <div className="flex items-center justify-end mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-gray-100 dark:border-gray-700">
              <button
                onClick={() => { setPostText(''); setShowComposer(false) }}
            ...
            {upcomingEvents.map(event => {
              ...
              return (
                <div key={event.id} className="flex items-center gap-2">
                  {event.image ? (
                    <img src={event.image} alt={event.title} className="w-8 h-8 rounded object-cover flex-shrink-0" />
                  ) : (
                    CategoryIcon && <CategoryIcon className="w-3 h-3 sm:w-4 sm:h-4 text-green-600 dark:text-green-400 flex-shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] sm:text-sm font-medium text-gray-800 dark:text-white truncate">{event.title}</p>
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        <button
          ...
          className={`px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium whitespace-nowrap transition ${selectedCategory === 'all' ? 'bg-prieska-terracotta text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:border-prieska-terracotta'}`}
        >
          All Events
        </button>
        {eventCategories.filter(c => c.id !== 'all').map(cat => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium whitespace-nowrap transition ${selectedCategory === cat.id ? 'bg-prieska-terracotta text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:border-prieska-terracotta'}`}
            >
            ...
            <div key={event.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
              ...
              <div className="p-2.5 sm:p-4 pb-1 sm:pb-2">
                <div className="flex items-start gap-2 sm:gap-3">
                  <div className={`p-1 sm:p-2 rounded-lg ${getCategoryColor(event.category)}`}>
                    {CategoryIcon && <CategoryIcon className="w-3 h-3 sm:w-4 sm:h-4" />}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-1 sm:gap-2">
                      <span className="text-[10px] sm:text-xs font-semibold text-gray-500 dark:text-gray-400">{getCategoryLabel(event.category)}</span>
                      <span className="text-[8px] sm:text-[10px] text-gray-400 dark:text-gray-500">•</span>
                      <span className="text-[8px] sm:text-[10px] text-gray-400 dark:text-gray-500">{getDateLabel(event.date)}</span>
                      {event.recurring && <>
                        <span className="text-[8px] sm:text-[10px] text-gray-400 dark:text-gray-500">•</span>
                        <span className="text-[8px] sm:text-[10px] text-gray-400 dark:text-gray-500">{event.recurring}</span>
                      </>}
                    </div>
                  </div>
                </div>
              ...
              {event.image && (
                <div className="px-2.5 sm:px-4 pb-2">
                  <img src={event.image} alt={event.title} className="w-full h-36 sm:h-44 object-cover rounded-lg" />
                </div>
              )}
